...
namespace xdp {
  ...
  // ************************
  TraceLogger::TraceLogger(ProfileCounters* profileCounters, TraceParser * TraceParserHandle, XDPPluginI* Plugin, std::shared_ptr<ProfilePlatform> profile_platform)
  : mMigrateMemCalls(0),
    mCurrentContextId(0),
  ...
  double TraceLogger::getDeviceTimeStamp(double hostTimeStamp, std::string& deviceName)
  {
    ...
    if (mPluginHandle->getFlowMode() == xdp::RTUtil::HW_EM) {
      ...
      size_t dts = mProfilePlatform->get_device_timestamp(deviceName);
